...
// Инициализация корзины (загрузка из localStorage или создание пустого массива)
let cartItems = JSON.parse(localStorage.getItem('cart')) || [];

// Элементы страницы
const cartItemsList = document.getElementById('cart-items'); // Список товаров в корзине
const cartTotal = document.getElementById('cart-total'); // Общая сумма
...
function updateCartDisplay() {
  ...
  cartItems.forEach(item => {
    ...
    li.textContent = `${item.tea} - ${item.price} руб.`;
...
function addToCart(productId) {
  ...
  if (product) {
    cartItems.push(product); // Добавление товара в корзину
    saveCartToLocalStorage(); // Сохранение корзины
